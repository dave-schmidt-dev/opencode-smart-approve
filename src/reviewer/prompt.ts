...
export function buildReviewerPrompt(input: ReviewerPromptInput): string {
  ...
  const prompt = [
    ...
    "Allow a pipeline or && chain only when every segment independently matches that read-only allow rule; the separators do not make an otherwise manual segment safe.",
